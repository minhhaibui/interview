...
window.DEVOPS_QUIZ = [
  {
    ...
    options: [
      'Container chạy chậm hơn VM vì phải qua thêm một lớp trung gian',
      'Container CHIA SẺ kernel host nên nhẹ, khởi động giây; VM ảo hoá cả OS',
      'VM nhẹ hơn container vì không cần đóng gói thư viện của ứng dụng',
      'Mỗi container cần một hệ điều hành khách riêng y hệt như VM',
    ], answer: 1,
  ...
  },
  {
    ...
    options: [
      'Không mang lại lợi ích gì, chỉ là cách Docker lưu file nội bộ',
      'Layer bất biến được CACHE & CHIA SẺ → build nhanh, tiết kiệm dung lượng',
      'Nhiều layer làm image chạy chậm vì phải ghép lại lúc khởi động',
      'Mỗi layer thực chất là một container riêng chạy song song với nhau',
    ], answer: 1,
  ...
  },
  {
    ...
    options: [
      'Chạy nhiều container cùng lúc từ một Dockerfile duy nhất',
      'Stage build có compiler, chỉ COPY artifact sang stage runtime gọn nhẹ',
      'Tăng số layer để tận dụng cache tốt hơn khi build lại nhiều lần',
      'Build song song trên nhiều CPU để rút ngắn thời gian tạo image',
    ], answer: 1,
  ...
  },
  {
    ...
    options: [
      'Để container khởi động nhanh hơn nhờ đọc dữ liệu từ đĩa host',
      'Container ephemeral — volume giữ dữ liệu ngoài vòng đời container',
      'Volume giúp image nhỏ hơn vì dữ liệu không nằm trong layer image',
      'Không thực sự cần volume, dữ liệu trong container vốn đã bền vững',
    ], answer: 1,
  ...
  },
  {
    ...
    options: [
      'Giống hệt nhau, dùng cái nào cũng cho kết quả chạy như nhau',
      'ENTRYPOINT: lệnh cố định; CMD: tham số mặc định, dễ bị ghi đè khi run',
      'CMD luôn chạy trước ENTRYPOINT rồi mới tới lệnh chính của image',
      'ENTRYPOINT chỉ dùng được với shell form, không dùng được exec form',
    ], answer: 1,
  ...
  },
  {
    ...
    options: [
      'Một máy chủ vật lý hoặc máy ảo tham gia vào cluster Kubernetes',
      'Đơn vị triển khai nhỏ nhất: 1+ container chia sẻ network & storage',
      'Một cluster con nằm bên trong cluster lớn để cách ly tài nguyên',
      'Chỉ là tên gọi khác của container theo thuật ngữ của Kubernetes',
    ], answer: 1,
  ...
  },
  {
    ...
    options: [
      'Deployment là một loại pod đặc biệt có thêm quyền tự khởi động lại',
      'Deployment khai báo trạng thái mong muốn, rolling update & rollback',
      'Deployment chỉ chạy được đúng một pod duy nhất tại một thời điểm',
      'Pod là cấp trên, nó quản lý và tạo ra các Deployment bên dưới',
    ], answer: 1,
  ...
  },
  {
    ...
    options: [
      'Chạy container và giám sát vòng đời của chúng trên từng node',
      'Điểm truy cập ỔN ĐỊNH (DNS + IP ảo) và load balance tới các pod',
      'Lưu trữ dữ liệu bền vững cho pod thông qua PersistentVolume',
      'Đóng gói image và đẩy lên registry mỗi khi có bản build mới',
    ], answer: 1,
  ...
  },
  {
    ...
    options: [
      'Giống nhau, đều kiểm tra sức khoẻ và khởi động lại pod khi lỗi',
      'Liveness fail → RESTART pod; readiness fail → tạm gỡ khỏi Service',
      'Cả hai đều restart pod, chỉ khác chu kỳ kiểm tra mặc định',
      'Readiness restart pod, còn liveness chỉ gỡ pod khỏi load balancer',
    ], answer: 1,
  ...
  },
  {
    ...
    options: [
      'Chạy pod với quyền cao hơn để đọc được biến môi trường hệ thống',
      'Tách config khỏi image: ConfigMap cho config thường, Secret cho dữ liệu nhạy cảm',
      'Lưu trữ dữ liệu database bền vững cho các pod trong namespace',
      'Cân bằng tải giữa các pod dựa trên cấu hình đã khai báo sẵn',
    ], answer: 1,
  ...
  },
  {
    ...
    options: [
      'Tăng RAM và CPU cấp cho từng pod hiện có khi tải tăng cao',
      'Tự động tăng/giảm SỐ POD theo tải (CPU/memory hoặc metric tuỳ chỉnh)',
      'Xoá bớt node trong cluster vào giờ thấp điểm để tiết kiệm chi phí',
      'Chỉ chạy một lần lúc deploy để đặt số replica ban đầu cho đúng',
    ], answer: 1,
  ...
  },
  {
    ...
    options: [
      'Để code ngắn gọn hơn và dễ đọc khi nhiều người cùng bảo trì',
      'Pod bị giết & tạo lại bất cứ lúc nào → đẩy state ra Redis/DB bên ngoài',
      'Stateless luôn làm ứng dụng chạy nhanh hơn trong mọi tình huống',
      'Không quan trọng lắm vì Kubernetes tự lưu lại trạng thái của pod',
    ], answer: 1,
