...
kill -9 $(lsof -ti :3000)   # lấy PID rồi kill`,
  ...
  },
  {
    ...
    options: ['git log --grep=bug', 'git bisect — chia đôi lịch sử, mỗi bước đánh dấu good/bad, ~log2(200) ≈ 8 lần thử là ra', 'git revert từng commit một', 'git diff HEAD~200'], answer: 1,
  ...
  },
  {
    ...
    options: [
      'curl -X POST http://localhost:3000/users -d \'{"name":"An"}\'',
      'curl http://localhost:3000/users --json-post name=An',
      'curl -X POST http://localhost:3000/users -H "Content-Type: application/json" -d \'{"name":"An"}\'',
      'curl -G http://localhost:3000/users?name=An',
    ], answer: 2,
  ...
  },
  {
    ...
    cmd: '> TTL session:9\n(integer) -1',
    options: [
      'Key TỒN TẠI nhưng KHÔNG đặt thời gian hết hạn (sống vĩnh viễn tới khi xoá/evict)',
      'Key đã hết hạn cách đây 1 giây',
      'Key không tồn tại',
      'Lỗi cú pháp lệnh',
    ], answer: 0,
