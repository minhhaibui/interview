...
window.CLI_QUIZ = [
  {
    ...
    options: [
      'Giống nhau hoàn toàn, chỉ khác tên tham số theo phiên bản git',
      '--soft lùi commit nhưng GIỮ thay đổi ở staging; --hard xoá sạch',
      '--soft xoá luôn thay đổi, còn --hard thì giữ lại ở working tree',
      '--hard chỉ đổi message của commit gần nhất, không đụng tới file',
    ], answer: 1,
  ...
  },
  {
    ...
    options: [
      'git reset --hard về commit trước rồi git push --force lên remote',
      'git revert <commit> — tạo commit mới đảo ngược, giữ nguyên lịch sử',
      'git rebase -i để xoá hẳn commit đó khỏi lịch sử rồi push lại',
      'git commit --amend để sửa lại nội dung của commit vừa push lên',
    ], answer: 1,
  ...
  },
  {
    ...
    options: [
      'Giống hệt nhau, pull chỉ là bí danh viết tắt của lệnh fetch',
      'fetch chỉ TẢI về remote-tracking; pull = fetch + merge (hoặc rebase)',
      'pull chỉ tải dữ liệu về, còn fetch mới thực sự gộp vào nhánh',
      'fetch dùng khi push, còn pull chỉ dùng lúc clone repo lần đầu',
    ], answer: 1,
  ...
  },
  {
    ...
    options: [
      'Cả hai trỏ tới cùng một commit, chỉ khác cách viết ký hiệu',
      'HEAD~2 = lùi 2 đời theo first-parent; HEAD^2 = parent THỨ HAI',
      'HEAD~2 là parent thứ hai, còn HEAD^2 là lùi hai đời tổ tiên',
      'Cả hai đều là cú pháp không hợp lệ trong git phiên bản mới',
    ], answer: 1,
  ...
  },
{
    ...
    options: [
      'Giống nhau, cờ -a chỉ đổi định dạng cột hiển thị cho gọn',
      'docker ps chỉ hiện container ĐANG chạy; -a hiện cả đã dừng',
      'Cờ -a khiến lệnh liệt kê image thay vì liệt kê container',
      'docker ps hiện tất cả, còn -a lọc container theo tên đặt sẵn',
    ], answer: 1,
  ...
  },
{
  ...
  },
  {
    ...
    options: [
      'Vì lệnh KEYS không hỗ trợ ký tự đại diện (wildcard) như *',
      'KEYS quét toàn bộ keyspace trong một lệnh, CHẶN Redis đơn luồng',
      'Vì lệnh KEYS chỉ chạy được trên replica, không chạy trên master',
      'Vì KEYS xoá luôn các key khớp mẫu ngay sau khi liệt kê xong',
    ], answer: 1,
  ...
  },
  {
    ...
    options: [
      'EXPLAIN — chỉ ước lượng kế hoạch, không thực sự chạy query',
      'EXPLAIN ANALYZE — chạy thật, đo thời gian & số hàng thực tế',
      'DESCRIBE SELECT ... để xem cấu trúc kết quả query trả về',
      'SHOW PLAN để in kế hoạch thực thi gần nhất của phiên hiện tại',
    ], answer: 1,
  ...
  },
  {
    ...
    options: [
      ...
      'find . -name "*.log" -type ERROR',
      'cat *.log | ERROR --recursive',
    ], answer: 1,
  ...
  },
  {
    ...
    cmd: 'kill -9 1234',
    options: [
      'kill -9 1234 — gửi SIGKILL, tiến trình không thể bắt hay bỏ qua',
      'kill -1 1234 — gửi SIGHUP, thường dùng để nạp lại cấu hình',
      'stop 1234 — dừng tiến trình theo PID rồi giải phóng tài nguyên',
      'kill -0 1234 — chỉ kiểm tra tiến trình còn sống hay đã chết',
    ], answer: 0,
...
git stash pop        # lấy lại thay đổi`,
    options: [
      'git commit tạm hết rồi reset lại sau khi quay về nhánh cũ',
      'git stash để cất tạm, xong việc thì git stash pop lấy lại',
      'Copy file ra ngoài repo, đổi nhánh rồi chép ngược trở lại',
      'git rm --cached toàn bộ file đang sửa rồi checkout nhánh khác',
    ], answer: 1,
...
kill -9 $(lsof -ti :3000)   # lấy PID rồi kill`,
    options: [
      'netstat -a | grep 3000 để xem cổng nào đang ở trạng thái mở',
      'lsof -i :3000 (hoặc ss -ltnp | grep 3000) để tìm PID giữ cổng',
      'ping localhost:3000 để kiểm tra cổng có ai đang nghe hay không',
      'grep 3000 /etc/services để tra xem dịch vụ nào chiếm cổng đó',
    ], answer: 1,
  ...
  },
  {
    ...
    options: [
      ...
      'kubectl proxy --port=8080 rồi gọi qua đường dẫn của pod',
      'kubectl get pod mypod -o wide để lấy IP rồi gọi thẳng vào',
    ], answer: 1,
  ...
  {
    id: 'cli-git-08', topic: 'git',
    q: 'Bug xuất hiện "đâu đó trong 200 commit gần đây", có cách tái hiện rõ. Công cụ git nào tìm ĐÚNG commit gây bug nhanh nhất?',
    cmd: 'git ___ start\ngit ___ bad            # bản hiện tại lỗi\ngit ___ good v1.2.0    # bản này còn tốt',
    options: [
      'git log --grep=bug để tìm commit có nhắc tới bug trong message',
      'git bisect — chia đôi lịch sử, khoảng 8 lần thử là ra commit lỗi',
      'git revert lần lượt từng commit cho tới khi bug biến mất hẳn',
      'git diff HEAD~200 để xem toàn bộ thay đổi rồi đọc dò tìm lỗi',
    ], answer: 1,
    explain: '`git bisect` tìm nhị phân trên lịch sử: đánh dấu 1 bản good + 1 bản bad, git tự checkout điểm giữa cho bạn test rồi thu hẹp dần — 200 commit chỉ cần ~8 lần thử. Có cả `git bisect run <script>` để tự động hoá hoàn toàn. Xong nhớ `git bisect reset`.',
  },
  {
    id: 'cli-curl-01', topic: 'bash',
    q: 'Gọi thử API tạo user bằng curl: POST JSON kèm header đúng. Lệnh nào CHUẨN?',
    options: [
      'curl -X POST http://localhost:3000/users -H "Accept: application/json" -d \'{"name":"An"}\'',
      'curl -X POST http://localhost:3000/users -H "Content-Type: text/plain" -d \'{"name":"An"}\'',
      'curl -X POST http://localhost:3000/users -H "Content-Type: application/json" -d \'{"name":"An"}\'',
      'curl -G http://localhost:3000/users -H "Content-Type: application/json" -d \'{"name":"An"}\'',
    ], answer: 2,
  ...
  },
  {
    ...
    cmd: '> TTL session:9\n(integer) -1',
    options: [
      'Key TỒN TẠI nhưng không đặt TTL — sống mãi tới khi bị xoá',
      'Key đã hết hạn cách đây đúng một giây và vừa bị Redis dọn đi',
      'Key không tồn tại trong keyspace hiện tại của Redis',
      'Lệnh sai cú pháp nên Redis trả về mã lỗi quy ước là -1',
    ], answer: 0,
    explain: 'Quy ước TTL: trả số giây còn lại nếu có expire; `-1` = key tồn tại nhưng KHÔNG có expire; `-2` = key KHÔNG tồn tại. Key session/cache mà TTL -1 thường là quên đặt EXPIRE — nguồn rò rỉ bộ nhớ Redis kinh điển.',
  },
  {
    id: 'cli-k8s-07', topic: 'kubectl',
    q: 'Pod bị crash và đã restart — `kubectl logs mypod` chỉ hiện log của container MỚI. Xem log của lần chạy TRƯỚC KHI crash bằng gì?',
    options: [
    ...
    explain: 'Cờ `--previous` (hoặc `-p`) lấy log của container instance TRƯỚC — nơi chứa stack trace/lý do crash thật. `--since` vẫn đọc container hiện tại; describe/events cho biết BỊ kill vì gì (OOMKilled, probe fail) nhưng không có log ứng dụng — hai lệnh bổ trợ nhau khi điều tra crash loop.',
  },
];
