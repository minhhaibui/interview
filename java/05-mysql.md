...
## 2. Clustered index & "back-to-table lookup" (tra ngược bảng)

- Bảng InnoDB **chính là** clustered index theo **khoá chính**: node lá chứa **toàn bộ hàng**.
- **Secondary index** (index phụ): node lá chỉ chứa `(giá trị index → khoá chính)`. Muốn lấy cột khác → phải dùng khoá chính **tra clustered index lần nữa** = **back-to-table lookup** (tốn thêm I/O).
- **Covering index** (index chứa đủ cột cần) → tránh được back-to-table.
...
2. Clustered index là gì? "back-to-table lookup" là gì và tránh bằng cách nào?
